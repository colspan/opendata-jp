...
tmp_out_file = "../var/A04_tmp_{}.csv"
out_file = "../var/A04_population_histgram_of_each_commune_agg_by_duration_brain.csv"
target_table = "hospitals_brain"
...
class generate_csv(luigi.Task):
    ...
    def run(self):
        with self.output().open("w") as f:
            print >> f, ",".join(["脳疾患取り扱い病院時間距離"] + ["人口({}～{}分)".format(x, x+DURATION_STEP) for x in duration_candidates] + ["人口率({}～{}分)".format(x, x+DURATION_STEP) for x in duration_candidates])
